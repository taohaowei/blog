...
public class LoadInterceptor implements HandlerInterceptor {
    ...
    @Autowired@Qualifier("viewLogMapper")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        ViewLog view = viewLogMapper.findViewLogByIp(request.getRemoteAddr());
        if(view!=null){
            request.setAttribute("visiter",1);
        }else{
            view = new ViewLog();
        }
        viewLog = new ViewLog(request.getRemoteAddr(),request.getRequestURI(),getUrlParams(request.getParameterMap()),request.getRequestedSessionId(),new Date(),view.getVisiter()+1);
        beginTime = System.currentTimeMillis();

        Ban ban = banMapper.isBan(viewLog.getIpAddress());
        if(ban!=null){
            return false;
        }
    ...
    public void postHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o, ModelAndView modelAndView) throws Exception {
        ...
        viewLog.setSpendTime(spendTime);
        ...
        while (iterator.hasNext())
        {
            ...
            for (int i=0;i<value.length;i++)
                if(i==0)
                    ...
                    sb2.append(","+value[i]);
            sb2.append("]");
            sb.append("("+next.getKey()+" : "+ sb2.toString() +"),");
        }
        sb.append("}");
        return sb.toString();
    }
